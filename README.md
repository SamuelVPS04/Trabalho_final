...
# 5. Banco de Dados
echo "✓ Banco de Dados:"
ls -lh instance/sistema.db 2>/dev/null || echo "Banco não existe (será criado ao iniciar)"
...
try:
    ...
    with app.app_context():
        ...
        print(f"✅ Users table OK ({users_count} usuários)")
...
```bash
# Solução 1: Verificar se ambiente virtual está ativado
source .venv/bin/activate

# Solução 2: Reinstalar dependências
pip install -r requirements.txt

# Solução 3: Verificar se requirements.txt existe
...
```bash
# Solução 1: Matar processo na porta 5000
lsof -i :5000
kill -9 <PID>
...
# Solução 3: Verificar se já está rodando
ps aux | grep python
```

### Erro: `No such file or directory: 'sistema.db'`

```bash
# Solução 1: Deixe o Flask criar automaticamente
python app.py  # Criará o banco
...
```bash
# Solução 1: Criar pasta de uploads
...
```bash
# Solução 1: Limpar cache pytest
rm -rf .pytest_cache
rm -rf __pycache__
pytest --cache-clear

# Solução 2: Rodar com mais informações
pytest -vvs

# Solução 3: Verificar se está no diretório correto
pwd  # Deve estar em Trabalho_final/
ls tests/
```
...
1. **Nunca commit .env** - Adicione ao `.gitignore`
2. **Nunca commit do banco** - Adicione `instance/` ao `.gitignore`
3. **Sempre use ambiente virtual** - Evita conflitos de versão
4. **Backup regular do banco** - Antes de mudanças importantes
5. **Teste antes de produção** - Execute `pytest` antes de deploy
...
2. Consulte [ANALISE_COMPLETA.md](ANALISE_COMPLETA.md)
